...
CREATE TABLE buildings (
	...
	photos JSON [],
	address TEXT,
	base_rental INTEGER NOT NULL CHECK (base_rental > 0), 
	base_cost_of_electricity INTEGER NOT NULL CHECK (base_cost_of_electricity > 0), 
	perodic_date NUMERIC CHECK (perodic_date > 0) DEFAULT(1),
...
INSERT INTO buildings (name, address,base_cost_of_electricity, base_rental,using_blocks_model, using_floors_model, status)
		values('block and floor binola', '991 Carpenter Drive',200,500, true, true, 'Active
		');
...
CREATE TABLE blocks (
	...
	photos TEXT [],
	base_rental INTEGER  CHECK (base_rental > 0), 
	base_cost_of_electricity INTEGER  CHECK (base_cost_of_electricity > 0), 
	status VARCHAR REFERENCES status (name),
	created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
	updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

INSERT INTO blocks (block_name, status, building_id,base_cost_of_electricity)
		values('C', 'Active', 2, 2000);
...
INSERT INTO rooms (room_number,floor_id,block_id, status, building_id,base_rental,base_cost_of_electricity)
		values('3', 1,2,'Active',2,50,100);
...
CREATE TABLE genders (
	id SMALLSERIAL CONSTRAINT genderId PRIMARY KEY,
